...
import { usePathname } from 'next/navigation'
import { motion, AnimatePresence, useScroll, useTransform } from 'framer-motion'
import { Menu, X, MessageCircle, Sun, Moon, ShoppingBag } from 'lucide-react'
import { useTheme } from 'next-themes'
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            className="fixed inset-0 z-40 lg:hidden flex flex-col bg-white dark:bg-[#080808]"
            initial={{ clipPath: 'inset(0 0 100% 0)' }}
            animate={{ clipPath: 'inset(0 0 0% 0)'   }}
            exit={{   clipPath: 'inset(0 0 100% 0)'   }}
            transition={{ duration: 0.55, ease: [0.16, 1, 0.3, 1] }}
          >
            ...
            {/* Gold orb */}
            <div
              className="absolute top-[-10%] right-[-10%] w-[60vw] aspect-square rounded-full pointer-events-none"
              style={{
                background: 'radial-gradient(circle, hsl(43 96% 56% / 0.1) 0%, transparent 65%)',
                filter: 'blur(60px)',
              }}
            ...
            <div className="relative z-10 flex items-center justify-between px-5 sm:px-8 pt-5">
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.button
                  ...
                  className="relative w-9 h-9 rounded-xl flex items-center justify-center
                    text-black/50 dark:text-white/50 hover:bg-black/5 dark:hover:bg-white/8
                    transition-colors"
                >
                  <ShoppingBag className="h-4 w-4" />
                  {totalCount > 0 && (
                    <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1
                      rounded-full bg-gold text-black text-[10px] font-bold
                      flex items-center justify-center">
                      {totalCount > 9 ? '9+' : totalCount}
                    </span>
                  )}
                ...
                return (
                  <div key={link.href} className="overflow-hidden border-b border-black/6 dark:border-white/6">
                    ...
                    >
                      ...
                      >
                        <span className="font-bebas text-[11vw] sm:text-7xl leading-none tracking-tight transition-colors duration-200">
                          {link.label}
                        </span>
                        {isActive && (
                          <span className="w-2 h-2 rounded-full bg-gold flex-shrink-0" />
