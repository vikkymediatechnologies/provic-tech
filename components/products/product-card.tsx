...
import { Star, MessageCircle, ArrowUpRight, ShoppingBag, Check } from 'lucide-react'
        ...
        <div className="flex items-center justify-between gap-3">
          <Link href={`/products/${product.id}`} className="flex-1 min-w-0">
            <div className="flex items-center gap-1">
              ...
              <ArrowUpRight className="w-3.5 h-3.5 text-black/20 dark:text-white/20
                group-hover:text-gold -translate-x-1 group-hover:translate-x-0
                opacity-0 group-hover:opacity-100 transition-all duration-300 shrink-0" />
            </div>
          </Link>

          {/* Add to cart */}
          <motion.button
            whileHover={{ scale: product.inStock ? 1.08 : 1 }}
            whileTap={{ scale: product.inStock ? 0.93 : 1 }}
